...
import { Book, PenTool, GraduationCap, Moon, Sun } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-16">
        ...
        <p className="text-xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">
          EduMind uses Gemini to create personalized study notes and interactive mock tests instantly. What do you want to achieve today?
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        >
          <div className="absolute top-0 right-0 p-4 opacity-5 dark:opacity-[0.03] group-hover:opacity-10 transition-opacity">
            <Book className="w-32 h-32 text-slate-900 dark:text-white" />
          </div>
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">Study Notes Generator</h2>
          <p className="text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
            Enter any topic and get comprehensive, structured notes with key concepts, definitions, and summaries formatted for easy reading.
          </p>
        ...
        >
          <div className="absolute top-0 right-0 p-4 opacity-5 dark:opacity-[0.03] group-hover:opacity-10 transition-opacity">
            <PenTool className="w-32 h-32 text-slate-900 dark:text-white" />
          </div>
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">Mock Test Creator</h2>
          <p className="text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
            Test your knowledge with AI-generated quizzes. Customize difficulty and question count to practice effectively.
          </p>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-indigo-100 dark:selection:bg-indigo-900 selection:text-indigo-900 dark:selection:text-indigo-100 transition-colors duration-300">
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 text-center text-slate-400 dark:text-slate-500 text-sm">
          <p>&copy; {new Date().getFullYear()} EduMind. AI answers can be inaccurate, please verify important information.</p>
        </div>
      </footer>
    </div>
  );
